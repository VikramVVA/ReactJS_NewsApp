        ...
        <>
            <div className="container mt-2  px-6 mx-auto border-8">
                <section className="mb-32 text-gray-1000">
                    <h2 className="text-3xl font-bold mb-12 text-center">Sports </h2>
                    <div className="grid sm:grid-cols-3 gap-6">
                      {data.filter((val)=>{
                          if(props.search===undefined){
                              return val
                          }
                        if(val.title.toLowerCase().includes(props.search&&props.search.toLowerCase())){
                            return val
                        }
                      }).map((val,i)=> {
                      return  (     <div key={i} className="relative overflow-hidden bg-no-repeat bg-cover shadow-lg rounded-lg" style={{ backgroundPosition: '50%' }} data-mdb-ripple="true" data-mdb-ripple-color="light">
                             <img src={val.urlToImage&&val.urlToImage} className="w-full" alt="" />
                             <Link   onClick={(e)=>handleset(e,val,i)}to={`/article/${i}`}>
